...
def move_custom(direction, steps):
    global current_position, step_counter_label  # Declare variables as global
    if direction == 'Forward':  # Check if the direction is forward
        ...
        current_position -= steps  # Decrement the current position by the number of steps
...
def move_to_zero():
    ...
    if steps_to_zero != 0:  # Check if there are steps needed to move to zero
        ser.write(b'B' + str(abs(steps_to_zero)).encode() + b'\n')  # Command to move backward to zero position
