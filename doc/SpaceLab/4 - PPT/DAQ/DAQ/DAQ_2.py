...
Lt = 0.1  # Example moment arm length in meters
k = 10.0  # Example spring constant in Newtons per meter
G = 0.05  # Example sensor responsivity in volts per meter
...
def convert_to_thrust():
    ...
    try:
        ...
        # Calculate sensitivity for steady-state measurement
        sensitivity_ss = Lt / k

        # Calculate sensitivity for impulse measurement
        sensitivity_impulse = Lt / 1  # Assuming impulse force of 1 Newton

        # Apply calibration to convert deflection measurements to thrust measurements
        thrust_data = deflection_data * sensitivity_ss  # Adjust for different scenarios if needed
