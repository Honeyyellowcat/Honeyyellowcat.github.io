...
# https://github.com/bkairosh/dataSender/blob/main/.gitignore
...
def convert_to_thrust():
    ...
    try:
        # Calculate the sensitivity (scale factor)
        ...
        # Error propagation: Propagate uncertainties from deflection to thrust
        deflection_std_dev = np.std(deflection_data)
        thrust_uncertainties = deflection_std_dev * sensitivity  # Scale the standard deviation

        uncertainties = np.full_like(thrust_data, thrust_uncertainties)  # Set uncertainties based on scaled standard deviation
...
record_button = ttk.Button(button_frame, text="Record Data", command=record_deflection)
record_button.grid(row=0, column=0, padx=5)

convert_button = ttk.Button(button_frame, text="Impulse (m) V.S Time (s) Graph", command=convert_to_thrust)
convert_button.grid(row=0, column=1, padx=5)

# STEP RESPONCE convert_button = ttk.Button(button_frame, text="Step Responce (m) V.S Time (s) Graph", command=convert_to_thrust)
#convert_button.grid(row=0, column=1, padx=5)

# SENSITIVITY onvert_button = ttk.Button(button_frame, text="Displacement V.S Time (s) Graph", command=convert_to_thrust)
# convert_button.grid(row=0, column=1, padx=5)

plot_button = ttk.Button(button_frame, text="Display Graph", command=display_graph)
plot_button.grid(row=0, column=2, padx=5)

export_button = ttk.Button(button_frame, text="Export Data and Graphs", command=export_data)
