...
def read_grid(file_path):
    ...
    try:
        with open(file_path, 'r') as file:
            for line in file:
                row = csv_line_to_list(line.strip())
                grid.append(row)
    except IOError as e:
        print(e)
    return grid
...
def average_of_surrounding(pixel_grid, i, j):
    ...
    for di in range(-1, 2):
        for dj in range(-1, 2):
            total += get_pixel_at(pixel_grid, i + di, j + dj)
            count += 1
    return total // count
...
def blur(pixel_grid):
    ...
    for i in range(len(pixel_grid)):
        ...
        for j in range(len(pixel_grid[0])):
            row.append(average_of_surrounding(pixel_grid, i, j))
        blurred_grid.append(row)
    return blurred_grid
...
blurred_image_grid = blur(input_grid)
write_image(output_image_filename, blurred_image_grid)
write_grid(output_grid_filename, blurred_image_grid)
