...
R_specific_CH4 = R_universal / molecular_mass_CH4  # J/(g*K)
R_specific_O2 = R_universal / molecular_mass_O2  # J/(g*K)

# Convert chamber pressure from psi to Pa
chamber_pressure_Pa = chamber_pressure_psi * 6894.76  # 1 psi = 6894.76 Pa
...
c_star = math.sqrt(R_universal * chamber_temperature_K / average_molecular_mass_amu) * 1000  # m/s
F_v = chamber_pressure_Pa * throat_diameter_inch * 0.0254 * math.pi / 4 * (1 + expansion_ratio_corrected**2) / (expansion_ratio_corrected * math.sqrt(2 * ratio_specific_heats_products / (ratio_specific_heats_products - 1) * (1 - (1 / expansion_ratio_corrected) ** ((ratio_specific_heats_products - 1) / ratio_specific_heats_products))))  # Newtons
I_sp_v = c_star / 9.81  # s

# Calculate corrected values for c_f_v (nozzle exit velocity)
c_f_v = math.sqrt((2 * ratio_specific_heats_products * R_universal * chamber_temperature_K) /
...
sea_level_I_sp_corrected = sea_level_I_sp * (math.sqrt(chamber_temperature_K / inlet_fuel_temperature_K) * (1 + (ratio_specific_heats_products - 1) / 2) ** ((ratio_specific_heats_products + 1) / (2 * (ratio_specific_heats_products - 1))))
sea_level_c_f = c_f_v * (sea_level_pressure_Pa / chamber_pressure_Pa)
...
print("   c_f_v: {:.2f} m/s".format(c_f_v))
...
print("   Sea Level I_sp: {:.2f} s".format(sea_level_I_sp))
